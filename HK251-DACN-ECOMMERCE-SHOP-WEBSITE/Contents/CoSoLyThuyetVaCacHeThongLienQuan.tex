...
\begin{itemize}
  ...
  \item \textbf{Tự động hóa quy trình xử lý đơn hàng:} Các bước từ đặt hàng, thanh toán, đến giao nhận được kết nối qua API với cổng thanh toán (VNPAY) và đơn vị vận chuyển (GHN, GHTK), giúp giảm thời gian xử lý.
...
Website kết hợp công nghệ hỗ trợ gợi các ý sản phẩm có thể mua chung với nhau để tạo thành một bộ phối đồ (Buy full set) cùng giao diện thân thiện,
giúp khách hàng dễ dàng lựa chọn trang phục phù hợp phong cách và vóc dáng.

\subsubsection{Tính năng nổi bật của Paple}
\begin{itemize}
  \item Buy full set: hỗ trợ hiển thị gợi ý các sản phẩm khác để cùng với sản phẩm đang xem tạo thành một bộ.
...
\subsection{GHN - Giao hàng nhanh}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.3\textwidth]{Images/Tech_logo/ghn-logo.png}
  \vspace{0.5cm}
  \caption{Logo của GHN}
  \label{fig: Logo của GHN}
\end{figure}

Đối với phân hệ xử lý giao hàng, hệ thống đã tích hợp dịch vụ Giao Hàng Nhanh (GHN) \cite{ghn2025} – đơn vị cung cấp giải pháp vận chuyển nhanh hàng đầu và phổ biến nhất tại Việt Nam hiện nay. GHN đóng vai trò là cầu nối logistics linh hoạt giữa cửa hàng trực tuyến và khách hàng trên toàn quốc, cho phép đơn hàng được xử lý và trao đến tay người mua một cách nhanh chóng thông qua đa dạng các tùy chọn dịch vụ như: Giao hàng tiêu chuẩn nội thành, giao hỏa tốc trong ngày, giao hàng thu tiền hộ (COD) và hỗ trợ đối soát tự động, minh bạch. \\

Về mặt kỹ thuật, GHN cung cấp bộ API chuẩn hóa theo kiến trúc RESTful, dễ dàng tích hợp vào hệ thống Backend (NestJS). Quy trình vận hành được thiết kế theo mô hình đồng bộ trạng thái qua Webhook, đảm bảo mọi thay đổi về tình trạng đơn hàng (đã tiếp nhận, đang lấy hàng, đang giao, giao thành công, hoàn trả) được cập nhật theo thời gian thực mà không cần truy vấn định kỳ. Đồng thời, cơ chế xác thực bằng Token và truyền tải dữ liệu qua HTTPS giúp bảo vệ an toàn các thông tin nhạy cảm như địa chỉ, số điện thoại của khách hàng; hệ thống bán hàng chỉ lưu trữ mã vận đơn và trạng thái đơn hàng, không can thiệp vào dữ liệu cá nhân trong quá trình vận chuyển, đáp ứng tốt các quy định về bảo mật dữ liệu hiện hành.\\